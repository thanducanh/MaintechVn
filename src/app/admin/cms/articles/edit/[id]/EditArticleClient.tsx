// 📍 File: src/app/admin/cms/articles/edit/[id]/EditArticleClient.tsx
"use client";
...
export default function EditArticleClient({ articleData }: { articleData: any }) {
    const router = useRouter();
    const { t, language } = useAdminSettings();
    const [loading, setLoading] = useState(false);
    const [imagePreview, setImagePreview] = useState<string | null>(articleData?.imageUrl || null);
    const [activeLangTab, setActiveLangTab] = useState<"vi" | "en">("vi");

    // Reactive form tracking states (initialized from loaded articleData)
    const [titleVi, setTitleVi] = useState(articleData?.title || "");
    const [titleEn, setTitleEn] = useState(articleData?.title_en || "");
    const [summaryVi, setSummaryVi] = useState(articleData?.summary || "");
    const [summaryEn, setSummaryEn] = useState(articleData?.summary_en || "");
    const [contentVi, setContentVi] = useState(articleData?.content || "");
    const [contentEn, setContentEn] = useState(articleData?.content_en || "");
    const [category, setCategory] = useState(articleData?.category || "TIN_TUC");
    const [status, setStatus] = useState(articleData?.status || "PUBLISHED");
    const [isDirty, setIsDirty] = useState(false);
    const [isSaved, setIsSaved] = useState(false);

    // Add required render log for verification
    useEffect(() => {
        console.log("EDIT_ARTICLE_RENDER_V3_COMPACT_I18N");
    }, []);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (articleData?.id) formData.append("id", articleData.id.toString());
        ...
        try {
            ...
            if (result?.success || !result?.error) { 
                setIsSaved(true);
                setIsDirty(false);
                toast.success(t.adminArticles.form.update + " " + (language === "en" ? "successfully!" : "thành công!"));
                router.push("/admin/cms/articles");
                router.refresh();
            } else {
                toast.error(result?.error || "Error updating article");
            }
        } catch {
            ...
            <div className="mx-auto flex h-full max-w-[1400px] flex-col px-6 py-5">
                ...
                <div className="mb-4 flex shrink-0 items-center justify-between pb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-extrabold text-slate-900 dark:text-slate-50 uppercase tracking-tight">{t.adminArticles.form.editTitle}</h1>
                        <span className="bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 text-[10px] font-bold px-2 py-0.5 rounded-lg shadow-sm">
                            ID: #{articleData?.id}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2.5">
                        ...
                        <button
                            onClick={() => (document.getElementById("edit-news-form") as HTMLFormElement).requestSubmit()}
                            disabled={loading}
                        ...
                        >
                            ...
                            {t.adminArticles.form.update}
                        </button>
                    </div>
                </div>

                <form id="edit-news-form" onSubmit={handleSubmit} className="grid flex-1 min-h-0 grid-cols-1 xl:grid-cols-[1fr_360px] gap-5 items-start">
                                    ...
                                    <>
                                        ...
                                        <div className="absolute inset-0 bg-black/45 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-3">
                                            ...
                                            <button 
                                                ...
                                                className="p-2 bg-white/20 hover:bg-red-650 backdrop-blur-sm rounded-xl text-red-155 transition-colors shadow" 
                                                title="Remove Image"
                                            >
                                                <Trash2 size={18} />
                                            </button>
                        ...
                        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-colors shrink-0 select-none">
                            ...
                            <div className="space-y-2 text-[11px] font-bold text-slate-500 dark:text-slate-400">
                                ...
                                <div className="flex items-center justify-between">
                                    <span>Admin:</span>
                                    {!hasRequiredFields ? (
                                        <span className="text-red-655 dark:text-red-400 bg-red-50 dark:bg-red-950/20 px-2 py-0.5 rounded border border-red-100 dark:border-red-900/30 scale-[0.95] origin-right font-bold">
                                            {language === "en" ? "Missing required fields" : "Thiếu dữ liệu bắt buộc"}
                                        </span>
                                    ) : loading ? (
                                        <span className="text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/20 px-2 py-0.5 rounded border border-blue-100 dark:border-red-900/30 scale-[0.95] origin-right font-bold">
                                        ...
                                        <span className="text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20 px-2 py-0.5 rounded border border-amber-100 dark:border-amber-900/30 scale-[0.95] origin-right font-bold">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    {isPublicVisible ? (
                                        ...
                                        <span className="text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900/30 px-2 py-0.5 rounded border border-slate-100 dark:border-slate-800/30 scale-[0.95] origin-right font-bold">
