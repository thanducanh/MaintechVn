// 📍 File: src/app/admin/cms/products/create/CreateProductClient.tsx
"use client";

import { useState, useRef } from "react";
import { useRouter } from "next/navigation";
import { CloudUpload, Box, Tag, Loader2 } from "lucide-react";
...
export default function CreateProductClient() {
    ...
    const [industry, setIndustry] = useState("THIET_BI_FB");
    const [nature, setNature] = useState("BOM_THUC_PHAM");
    ...
    const handleSave = async () => {
        ...
        if (!nameVal || nameVal.trim() === "") {
            toast.error(language === 'en' ? "Please enter product name!" : "Chủ tịch quên nhập Tên sản phẩm rồi!");
            return;
        }
        ...
        try {
            ...
            if (res?.success) {
                setIsSaved(true);
                setIsDirty(false);
                toast.success(language === 'en' ? "Product saved successfully!" : "Lưu sản phẩm thành công rực rỡ!");
                router.push("/admin/cms/products");
                router.refresh();
            } else {
                toast.error(res?.error || (language === 'en' ? "Database rejected save!" : "Cơ sở dữ liệu từ chối lưu!"));
            }
        } catch (error) {
                    ...
                    <div className="flex items-center gap-3">
                        <div>
                            <h1 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight">
                                {language === 'en' ? 'ADD NEW PRODUCT' : 'THÊM SẢN PHẨM MỚI'}
                            </h1>
                            <p className="text-xs text-slate-450 dark:text-slate-400">
                                {language === 'en' ? 'Create a new bilingual industrial product entry' : 'Tạo mục nhập sản phẩm kỹ thuật song ngữ mới'}
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <button onClick={handleSave} disabled={loading} className="flex-1 sm:flex-none bg-indigo-600 text-white hover:bg-indigo-700 dark:bg-indigo-650 dark:hover:bg-indigo-700 px-5 py-2 rounded-lg font-bold text-[12px] shadow-md flex items-center justify-center gap-2 disabled:opacity-50 uppercase tracking-widest transition-all">
                            {loading ? (
                                ...
                                language === 'en' ? 'SAVE PRODUCT' : 'LƯU SẢN PHẨM'
                            )}
                        </button>
                    </div>
                </div>

                <form ref={formRef} className="grid min-h-0 flex-1 grid-cols-1 xl:grid-cols-[1fr_360px] gap-4 overflow-hidden">
                        ...
                        <div className="h-full">
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm h-full p-4 flex flex-col overflow-hidden transition-colors">
                                ...
                                <div className="flex-1 min-h-0 flex flex-col gap-3 mt-3">
                                    <label className="relative flex flex-col items-center justify-center w-full flex-1 min-h-[220px] bg-slate-50 dark:bg-slate-950 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-500 transition-all overflow-hidden shadow-inner group">
                                        {imagePreview ? (
                                            <img src={imagePreview} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                        ) : (
                                            <div className="text-center p-3">
                                                <CloudUpload size={24} className="text-slate-300 dark:text-slate-500 mx-auto mb-2 group-hover:text-indigo-500 transition-colors" />
                                                ...
                                                <p className="text-[8px] text-slate-300 dark:text-slate-600 mt-0.5">
                                                    {language === 'en' ? 'Supports JPG, PNG, WEBP' : 'Hỗ trợ JPG, PNG, WEBP'}
                                                </p>
                                            </div>
                                        )}
                                        <input 
