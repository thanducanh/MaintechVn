...
export default async function ServiceSlugPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const service = (await getDisplayServicesAction()).find((item: any) => String(item.slug) === slug);
  if (!service) notFound();
  let localized: any = {};
  try { localized = JSON.parse(service.content || "{}"); } catch { localized = {}; }
  const pageConfig: any = await getServicesPageConfigAction();
  const detail = pageConfig?.serviceDetails?.[service.slug] || {};
  ...
  const related = (await getDisplayServicesAction()).filter((item: any) => item.slug !== service.slug).slice(0, 3);
